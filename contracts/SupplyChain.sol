...
contract SupplyChain {
    // <owner>
    /**Set the owner of the supply chain */
    address public owner;
    // <skuCount>
    /**Sku to track the up-to-date sku number */
    ...
    modifier forSale(uint _sku) {
        require(
            (items[_sku].seller != address(0) && items[_sku].state == State.ForSale),
            "The item should be on sale in order to be bought"
        );
        _;
    }

    modifier sold(uint256 _sku) {
        require(items[_sku].state == State.Sold, "The item has sold");
        _;
    }

    modifier shipped(uint256 _sku) {
        require(items[_sku].state == State.Shipped, "The item has Shipped");
        _;
    }

    modifier received(uint256 _sku) {
        require(items[_sku].state == State.Received, "The item was Received");
        _;
    }
    ...
    function buyItem(uint256 sku) public payable forSale(sku) paidEnough(sku) {
        // Assign address that calls the function as a buyer
